...
import {
    score, attempts, currentTense, tenseStats,
    currentAttempt, remainingAttempts, isCorrect, showAnswer,
    feedback, userAnswer, convertedAnswer, correctAnswer
...
export function checkUserAnswer(useRomajiInput: boolean): void {
    ...
    userAnswer.subscribe(value => { userAnswerValue = value; })();
    correctAnswer.subscribe(value => { correctAnswerValue = value; })();
    currentTense.subscribe(value => { currentTenseValue = value; })();
    tenseStats.subscribe(value => { statsValue = value; })();
    currentAttempt.subscribe(value => { currentAttemptValue = value; })();
    ...
    if (checkConjugationAnswer(correctAnswerValue, convertedAnswerValue)) {
        ...
        remainingAttempts.subscribe(value => { remainingAttemptsValue = value; })();

        if (remainingAttemptsValue > 0) {
            feedback.set(`不正解 (Incorrect) - ${remainingAttemptsValue} ${remainingAttemptsValue === 1 ? 'attempt' : 'attempts'} remaining`);
